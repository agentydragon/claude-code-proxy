...
def get_config_path(config_file: Optional[str] = None) -> Path:
    ...
    # Fall back to ~/.config/claude-code-proxy/config.json
    if not config_path.exists():
        legacy_path = Path.home() / ".config" / "claude-code-proxy" / "config.json"
...
def save_example_config(output_path: Optional[str] = None):
    ...
    output = Path(output_path) if output_path else Path("config.example.json")
